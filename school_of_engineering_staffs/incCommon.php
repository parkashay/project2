<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'staffs' => "`staffs`.`id` as 'id', `staffs`.`name` as 'name', `staffs`.`title` as 'title', `staffs`.`photo` as 'photo', `staffs`.`description` as 'description', `staffs`.`research_paper` as 'research_paper'",
			'notes' => "`notes`.`id` as 'id', `notes`.`name` as 'name', IF(    CHAR_LENGTH(`notes_category1`.`notes_category`), CONCAT_WS('',   `notes_category1`.`notes_category`), '') as 'category', IF(    CHAR_LENGTH(`staffs1`.`name`), CONCAT_WS('',   `staffs1`.`name`), '') as 'uploaded_by', `notes`.`description` as 'description', `notes`.`photo` as 'photo', `notes`.`file_one` as 'file_one', `notes`.`file_two` as 'file_two'",
			'notes_category' => "`notes_category`.`id` as 'id', `notes_category`.`notes_category` as 'notes_category'",
			'student_achievments' => "`student_achievments`.`id` as 'id', `student_achievments`.`name` as 'name', `student_achievments`.`photo` as 'photo', `student_achievments`.`description` as 'description'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'staffs' => "`staffs` ",
			'notes' => "`notes` LEFT JOIN `notes_category` as notes_category1 ON `notes_category1`.`id`=`notes`.`category` LEFT JOIN `staffs` as staffs1 ON `staffs1`.`id`=`notes`.`uploaded_by` ",
			'notes_category' => "`notes_category` ",
			'student_achievments' => "`student_achievments` ",
		];
...
EOT;
	...
	function StyleSheet() {
		...
		$css_links = <<<EOT
			...
			<link rel="stylesheet" href="{$prepend_path}resources/initializr/css/readable.css">
