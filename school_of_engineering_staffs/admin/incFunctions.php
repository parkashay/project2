	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'staffs' => ['Staffs', '', 'table.gif', 'None'],
			'notes' => ['Notes', '', 'table.gif', 'None'],
			'notes_category' => ['Notes category', '', 'table.gif', 'None'],
			'student_achievments' => ['Student achievments', '', 'table.gif', 'None'],
		];
	...
	function getThumbnailSpecs($tableName, $fieldName, $view) {
		if($tableName=='staffs' && $fieldName=='photo' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='staffs' && $fieldName=='photo' && $view=='dv')
			return ['width'=>250, 'height'=>250, 'identifier'=>'_dv'];
		elseif($tableName=='notes' && $fieldName=='photo' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='notes' && $fieldName=='photo' && $view=='dv')
			return ['width'=>250, 'height'=>250, 'identifier'=>'_dv'];
		elseif($tableName=='student_achievments' && $fieldName=='photo' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='student_achievments' && $fieldName=='photo' && $view=='dv')
			return ['width'=>250, 'height'=>250, 'identifier'=>'_dv'];
		return FALSE;
	}
	########################################################################
			...
			$schema = [
				...
				'notes' => [
					...
					'file_two' => [
						...
						'info' => [
							'caption' => 'File two',
							'description' => 'Maximum file size allowed: 100000 KB.<br>Allowed file types: ppt, pptx, pptm, pdf, ppsx, ppsm, pps, odp',
						],
					],
				],
				'notes_category' => [
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'notes' => [
				'category' => 'SELECT `notes_category`.`id`, `notes_category`.`notes_category` FROM `notes_category` ORDER BY 2',
				'uploaded_by' => 'SELECT `staffs`.`id`, `staffs`.`name` FROM `staffs` ORDER BY 2',
			],
			'notes_category' => [
			],
			'student_achievments' => [
