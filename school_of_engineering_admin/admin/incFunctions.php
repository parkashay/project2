	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'soe_posts' => ['Soe posts', '', 'table.gif', 'None'],
			'category' => ['Category', '', 'table.gif', 'None'],
			'soe_gallery' => ['Soe Gallery', '', 'table.gif', 'None'],
			'courses' => ['Courses', '', 'table.gif', 'None'],
		];
	...
	function getThumbnailSpecs($tableName, $fieldName, $view) {
		if($tableName=='soe_gallery' && $fieldName=='file_photo' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='soe_gallery' && $fieldName=='file_photo' && $view=='dv')
			return ['width'=>250, 'height'=>250, 'identifier'=>'_dv'];
		elseif($tableName=='courses' && $fieldName=='faculty_photo' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='courses' && $fieldName=='faculty_photo' && $view=='dv')
			return ['width'=>250, 'height'=>250, 'identifier'=>'_dv'];
		return FALSE;
	}
	########################################################################
			...
			$schema = [
				'soe_posts' => [
					...
					'tags' => [
						...
						'info' => [
							'caption' => 'Tags',
							'description' => 'Separate the tags by comma(,).',
						],
					],
					'category' => [
				...
				'soe_gallery' => [
					...
					'year' => [
						...
						'info' => [
							'caption' => 'Year',
							'description' => 'year of photo taken in A.D.(like 2022)',
						],
					],
					'file_photo' => [
						'appgini' => "VARCHAR(40) NULL",
					...
					'file_video' => [
						...
						'info' => [
							'caption' => 'File video',
							'description' => 'Maximum file size allowed: 100000 KB.<br>Allowed file types: mov, avi, swf, asf, wmv, mpg, mpeg, mp4, flv',
						],
					],
					'youtube_link' => [
						'appgini' => "VARCHAR(40) NULL",
				...
				'courses' => [
					...
					'faculty_description' => [
						'appgini' => "TEXT NOT NULL",
						'info' => [
							'caption' => 'Faculty description',
							'description' => '',
						],
					],
					'contact' => [
						'appgini' => "BIGINT(15) NOT NULL",
						'info' => [
							'caption' => 'Contact',
							'description' => '',
						],
					...
					'location_google_map_key' => [
						'appgini' => "VARCHAR(40) NULL",
						'info' => [
							'caption' => 'Google map iframe',
							'description' => 'Learn how to add here:<br>https://www.maps.ie/create-google-map/',
						],
					],
					'syllabus' => [
						'appgini' => "TEXT NULL",
	...
	*/
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			'soe_posts' => [
				'category' => 'SELECT `category`.`id`, `category`.`category_name` FROM `category` ORDER BY 2',
			],
			'category' => [
			],
			'soe_gallery' => [
