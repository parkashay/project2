...
if (is_array($arrTables) && count($arrTables)) {
	...
	foreach ($tg as $tn => $tgroup) {
		...
		$tChkAHAN = array_search($tn, ['soe_posts', 'category', 'courses']);
...
?>
		...
		<div class="collapse" id="group-<?php echo md5($tgroup); ?>">
			...
			<?php if ($tChkHL === false || $tChkHL === null) { /* if table is not set as hidden in homepage */ ?>
				<div id="<?php echo $tn; ?>-tile" class="<?php echo (!$i ? $block_classes['first']['grid_column'] : $block_classes['other']['grid_column']); ?> dashboard-container">
					<div class="panel <?php echo (!$i ? $block_classes['first']['panel'] : $block_classes['other']['panel']); ?> dashboard-container-2">
						<div class="panel-body">
							<?php if ($can_insert && $tChkAHAN !== false && $tChkAHAN !== null) { ?>

								<div class="badge-container g-center"><?php echo $tc['Caption']; ?></div>
								<div class="dash-add-group-btn">
									<a id="<?php echo $tn; ?>_add_new" style="" class="  dash-link-btn1" title="<?php echo html_attr($Translation['Add New']); ?>" href="<?php echo $tn; ?>_view.php?addNew_x=1"><i class="fa fa-plus" aria-hidden="true"></i>
									</a>
									<a class=" dash-link-btn2" title="<?php echo preg_replace("/&amp;(#[0-9]+|[a-z]+);/i", "&$1;", html_attr(strip_tags($tc['Description']))); ?> dash-link-btn" href="<?php echo $tn; ?>_view.php<?php echo $searchFirst; ?>"><strong class="table-caption"><i class="fa fa-eye" aria-hidden="true"></i>
											View</strong></a>
								</div>
								<br>
								<div class="badge-container">Count: <?php echo $count_badge; ?></div>
							<?php } else { ?>
...
<script>
	$j(function() {
		...
		if (!table_descriptions_exist) {
			$j('div[id$="-tile"] .panel-body-description').css({
				height: 'auto'
			});
		}
