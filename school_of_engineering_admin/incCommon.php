<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'soe_posts' => "`soe_posts`.`id` as 'id', `soe_posts`.`title` as 'title', `soe_posts`.`tags` as 'tags', IF(    CHAR_LENGTH(`category1`.`category_name`), CONCAT_WS('',   `category1`.`category_name`), '') as 'category', if(`soe_posts`.`date`,date_format(`soe_posts`.`date`,'%m/%d/%Y'),'') as 'date', `soe_posts`.`content` as 'content', `soe_posts`.`file_name` as 'file_name', `soe_posts`.`photo` as 'photo'",
			'category' => "`category`.`id` as 'id', `category`.`category_name` as 'category_name'",
			'soe_gallery' => "`soe_gallery`.`id` as 'id', `soe_gallery`.`event_name` as 'event_name', `soe_gallery`.`year` as 'year', `soe_gallery`.`file_photo` as 'file_photo', `soe_gallery`.`file_video` as 'file_video', `soe_gallery`.`youtube_link` as 'youtube_link', `soe_gallery`.`year_calendar` as 'year_calendar', `soe_gallery`.`description` as 'description'",
			'courses' => "`courses`.`id` as 'id', `courses`.`faculty_name` as 'faculty_name', `courses`.`faculty_photo` as 'faculty_photo', `courses`.`faculty_description` as 'faculty_description', `courses`.`contact` as 'contact', `courses`.`email` as 'email', `courses`.`location_google_map_key` as 'location_google_map_key', `courses`.`syllabus` as 'syllabus'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'soe_posts' => "`soe_posts` LEFT JOIN `category` as category1 ON `category1`.`id`=`soe_posts`.`category` ",
			'category' => "`category` ",
			'soe_gallery' => "`soe_gallery` ",
			'courses' => "`courses` ",
		];
