...
$start = ($page - 1) * $limit;
$result = $conn->query("SELECT * FROM soe_posts where category= $category ORDER BY id DESC LIMIT $start,$limit");
...
$sql = "SELECT * FROM soe_posts where category='$category'";
    ...
    <main class="cd-main-contentS">
        <section class="component-main component-main-posts">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><a href="./allpost.php">News</a></li>
                <li class="breadcrumb-item active" aria-current="page"><?php echo $category_name; ?></li>
            </ol>
            <h1 class="section-heading highlight title "><?php echo $category_name; ?></h1>
        ...
        <ul class="pagination-btn-container" style="list-style: none;">
            <li class="pagination-btn btn-prev">
                <a <?php if ($page != 1) : ?> class="active-btn" href="category.php?page=<?= $previous; ?>" <?php endif; ?> class="pagination-link-btn"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
            </li>
            <?php for ($i = 1; $i <= $pages; $i++) : ?>
                <li class="pagination-btn btn-start">
                    <a <?php if ($i != $page) : ?> class="active-btn" href="category.php?page=<?= $i; ?>" <?php endif; ?> class="pagination-link-btn"><?= $i; ?></a>
                </li>
            <?php endfor; ?>
            <li class="pagination-btn btn-next">
                <a <?php if ($pages != $page) : ?> class="active-btn" href="category.php?page=<?= $next; ?>" <?php endif; ?> class="pagination-link-btn"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            </li>
        </ul>
        </section>
        <?php
