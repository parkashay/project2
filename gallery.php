...
$start = ($page - 1) * $limit;
$result = $conn->query("SELECT file_photo,event_name,year FROM soe_gallery ORDER BY id DESC LIMIT $start,$limit");
...
<body>
    ...
    <main class="cd-main-contentS">
        <section class="component-main component-main-posts">
            ...
            <h1 class="section-heading highlight title ">SOE Image Gallery</h1>
            <div class="box-posts-container">
                <?php foreach ($posts as $post) : ?>
                    <?php if (!empty($post['file_photo'])) : ?>
                        <!-- for thumbnail -->
                        <?php
                        $thumb_photo = $post['file_photo'];
                        ...
                        <div class="box-post">
                            <div class="box-post-title">
                                <div class="post-thumb-img spotlight" data-src="./school_of_engineering_admin/images/<?php echo $post['file_photo'] ?>">
                                    <img src="./school_of_engineering_admin/images/<?= $thumb_photo_link; ?>" alt="<?php echo $post['event_name'] ?>">
                                </div>
                            </div>
                            <div class="box-post-title" style="padding:5px; text-align:center;line-height:1.2;">
                                <?= $post['event_name']; ?>
                            </div>
                            <div class="box-post-date">
                                Uploaded In: <?= $post['year']; ?>
                            </div>
                        </div>
                    <?php endif; ?>
                <?php endforeach; ?>
            ...
            <ul class="pagination-btn-container" style="list-style: none;">
                <li class="pagination-btn btn-prev">
                    <a <?php if ($page != 1) : ?> class="active-btn" href="gallery.php?page=<?= $previous; ?>" <?php endif; ?> class="pagination-link-btn"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
                </li>
                <?php for ($i = 1; $i <= $pages; $i++) : ?>
                    <li class="pagination-btn btn-start">
                        <a <?php if ($i != $page) : ?> class="active-btn" href="gallery.php?page=<?= $i; ?>" <?php endif; ?> class="pagination-link-btn"><?= $i; ?></a>
                    </li>
                <?php endfor; ?>
                <li class="pagination-btn btn-next">
                    <a <?php if ($pages != $page) : ?> class="active-btn" href="gallery.php?page=<?= $next; ?>" <?php endif; ?> class="pagination-link-btn"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                </li>
            </ul>
        </section>
        <?php
