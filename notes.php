...
include("./fetch_data/fetch_departments.php");
$category = $_REQUEST['id'];
$category_name = $_REQUEST['subject'];
$numberOfItemsPerPage = 30;
$limit = isset($_POST['limit-records']) ? $_POST['limit-records'] : $numberOfItemsPerPage;
$page = isset($_GET['page']) ? $_GET['page'] : 1;
$start = ($page - 1) * $limit;
$sqlquery = "SELECT notes.id,notes.name,notes_category.notes_category FROM notes LEFT JOIN notes_category ON notes.category=notes_category.id WHERE notes.category='$category' ORDER BY notes.id DESC LIMIT $start,$limit";
...
<head>
    ...
    <title><?php echo $category_name; ?>-SOE</title>
</head>
...
<body>
    ...
    <main class="cd-main-contentS">
        <section class="component-main component-main-posts">
            ...
            <h1 class="section-heading highlight title ">Study Materials</h1>
            <div class="box-posts-container">
                <?php getnotescategory($sqlquery); ?>
            </div>
            <ul class="pagination-btn-container" style="list-style-type: none;">
                <li class="pagination-btn btn-prev">
                    <a <?php if ($page != 1) : ?> class="active-btn" href="notes.php?page=<?= $previous; ?>" <?php endif; ?> class="pagination-link-btn"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
                </li>
                <?php for ($i = 1; $i <= $pages; $i++) : ?>
                    <li class="pagination-btn btn-start">
                        <a <?php if ($i != $page) : ?> class="active-btn" href="notes.php?page=<?= $i; ?>" <?php endif; ?> class="pagination-link-btn"><?= $i; ?></a>
                    </li>
                <?php endfor; ?>
                <li class="pagination-btn btn-next">
                    <a <?php if ($pages != $page) : ?> class="active-btn" href="notes.php?page=<?= $next; ?>" <?php endif; ?> class="pagination-link-btn"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                </li>
            </ul>
        </section>
        <?php
